 ...
 * 
 * Thin horizontal authority bar before footer
 * Senior Level: Builds psychological authority
 */
...
const ComplianceBar = () => {
  const badges = [
    { label: 'SOLAS', icon: 'shield' },
    { label: 'IMO', icon: 'certificate' },
    { label: 'ISO', icon: 'check' },
    { label: 'Class Approved', icon: 'anchor' },
    { label: 'Global Sourcing Network', icon: 'globe' }
  ];
        ...
        <div className="compliance-content">
          {badges.map((badge, index) => (
            <div key={index} className="compliance-item">
              ...
              <span className="compliance-label">{badge.label}</span>
              {index < badges.length - 1 && <span className="compliance-separator">|</span>}
            </div>
          ))}
        </div>
      </div>
